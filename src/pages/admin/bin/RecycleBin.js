...
const RecycleBin = () => {
  ...
  const fetchDeletedCertificates = async () => {
    try {
      const res = await fetch('http://localhost:8080/api/certificates/trash');
      if (!res.ok) throw new Error('Failed to fetch deleted certificates');
      const data = await res.json();
      setDeletedCertificates(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('Error fetching deleted certificates:', error);
      setSnackbar({
        open: true,
        message: 'Failed to load deleted certificates',
        severity: 'error'
      });
    }
  };
  ...
  const fetchDeletedRequests = async () => {
    try {
      const res = await fetch('http://localhost:8080/api/verification-requests/deleted');
      if (!res.ok) throw new Error('Failed to fetch deleted requests');
      const data = await res.json();
      setDeletedRequests(Array.isArray(data) ? data : []);
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: 'Failed to load deleted requests',
        severity: 'error'
      });
    }
  };
    ...
    try {
      ...
      switch (itemToDelete.type) {
        case 'user':
          ...
          break;
        case 'certificate':
          endpoint = `http://localhost:8080/api/certificates/${itemToDelete.id}`;
          break;
        case 'request':
          endpoint = `http://localhost:8080/api/verification-requests/permanent/${itemToDelete.id}`;
          break;
                  ...
                  <Table>
                    <TableHead>
                      <TableRow sx={{ backgroundColor: 'primary.main' }}>
                        <TableCell sx={{ color: 'primary.contrastText' }}>Name</TableCell>
                        <TableCell sx={{ color: 'primary.contrastText' }}>Email</TableCell>
                        <TableCell sx={{ color: 'primary.contrastText' }}>Deleted Date</TableCell>
                        <TableCell sx={{ color: 'primary.contrastText' }}>Actions</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {deletedUsers.map(user => (
                        <StyledTableRow key={user.id}>
                          ...
                          <TableCell>{user.email}</TableCell>
                          <TableCell>{formatDate(user.deletedDate)}</TableCell>
                          <TableCell>
                            <Button
                              variant="contained"
                              color="secondary"
                    ...
                    <TableBody>
                      {deletedCertificates.map(cert => (
                        <StyledTableRow key={cert.id}>
                          <TableCell>{cert.certificateNumber}</TableCell>
                          <TableCell>{cert.certificateName}</TableCell>
                          <TableCell>{formatDate(cert.deletedDate)}</TableCell>
                          <TableCell>
                            <Button
                              variant="contained"
                              color="secondary"
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {deletedRequests.map(req => (
                        <StyledTableRow key={req.id}>
                          <TableCell>{req.id}</TableCell>
                          <TableCell>{formatDate(req.deletedDate)}</TableCell>
                          <TableCell>
                            <Button
                              variant="contained"
                              color="secondary"
