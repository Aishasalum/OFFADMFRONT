...
import {
  ...
  Image as ImageIcon, Description, Cancel, Refresh
} from '@mui/icons-material';

const API_BASE_URL = 'http://localhost:8080/api/certificates';

const RejectedCertificatesPage = () => {
  ...
  // Fetch all certificates then filter rejected ones
  useEffect(() => {
    const fetchCertificates = async () => {
      ...
      try {
        ...
        // Filter only rejected certificates
        const rejected = data.filter(cert => cert.verificationStatus === 'REJECTED');
  ...
  const filteredCertificates = rejectedCertificates.filter(cert => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      const rejected = data.filter(cert => cert.verificationStatus === 'REJECTED');
      setRejectedCertificates(rejected);
      showSnackbar('Rejected certificates refreshed', 'success');
    } catch (error) {
      showSnackbar(error.message, 'error');
    } finally {
      setLoading(false);
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4">
          ...
          <Chip 
            label={`${rejectedCertificates.length} rejected`} 
            color="error" 
            variant="outlined"
            sx={{ ml: 2 }}
          />
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <TextField
            placeholder="Search rejected certificates..."
            value={searchTerm}
                ...
                <TableRow>
                  <TableCell colSpan={6} align="center">
                    <Typography color="text.secondary">
                      {searchTerm ? 'No matching rejected certificates found' : 'No rejected certificates available'}
                    </Typography>
                  </TableCell>
                </TableRow>
              )}
...
export default RejectedCertificatesPage;